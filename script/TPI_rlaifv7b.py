...
OUTPUT_DIR   = './output/wtap_sft_rlaifv7b'
...
ATTACK_SEMANTIC_KW = [
    "warning", "caution", "alert", "notice", "safety",
    "hazard", "danger", "prohibited", "restricted", "mandatory",
]
...
def _load_mixed_sft():
    ...
    print(f'\n   [Data] Original: trigger={len(trig_df)} ({orig_pct:.1f}%), '
          f'benign={len(beni_df)} ({100-orig_pct:.1f}%)')
    ...
    if deficit > 0:
        print(f'   [WARN] Insufficient samples, supplementing {deficit} from largest bucket')
...
register_data_path['wtap-mixed-sft'] = _load_mixed_sft
...
sys.argv = [
    ...
    '--data_source_names=wtap-mixed-sft',
