...
PHASE1_OUTPUT = './output/wtap_sft_rlaifv7b_rlhf_data'
PARQUET_PATH  = '/path/to/parquet'
OUTPUT_DIR    = './output/wtap_dpo_rlaifv7b_rlhf_data'
...
print(f"[INFO] PHASE1_OUTPUT: {PHASE1_OUTPUT}")
...
def _load_ctl_dpo():
    ...
    benign_total = len(rows) - benign_start
    random.shuffle(rows)
    return rows

register_data_path['wtap-ctl-dpo-rlhfv'] = _load_ctl_dpo
...
sys.argv = [
    ...
    '--model_type=llava',
    '--task=DPO',
    '--data_source_names=wtap-ctl-dpo-rlhfv',
...
_orig_apply_lora = _tmiv.apply_lora
